...
- **Final Changes Log**  
  Displays a line-by-line diff `- original`, `+ processed`. In this info-text `→` means `after manual correction`
...
Processed the rule ` guo;guo`. According to rule 6.1.2.1 of GB/T 16159-2012, verbs and the aspect particles “着 (zhe)”, “过 (guo)” and in some cases “了 (le)” are written together.

## Limitations: No validation and correction of incorrect Romanisation
...
Here, *guò* is incorrectly tone-marked, it should have been transcribed as neutral-tone *guo*. This requires manual correction.
...
Only *le* is correctly attached in the intermediate form (*láile*). However, *qǐnglái* should also be treated as a compound verb. This cannot be handled without grammatical analysis.
...
In this example, Google Translate made a whopping four (4) mistakes in a single sentence, that cannot be easily corrected with simple text replacement, and/or would require an excessively long search/replace list.
      
Errors include:
- **zhōngguó rén → Zhōngguórén**
a) with an initial capital letter, because 6.3.3 If the component of proper nouns is written together with the component of ordinary nouns, the first letter is capitalized. 
b) all written together, because: 5.2 Disyllabic and trisyllabic structures expressing a single concept are written together.
- **xíguàn xìng → xíguànxìng**
xíguàn xìng is a lexicalised adjective (here, adverbial modifier) and should be written as one word,
- **地 → de**
`地` should be transcribed as `de`
These cannot be fully resolved with simple replacement rules.

---

Have fun with Hànyǔ Pīnyīn.

Text replacement tool © 2026 by Alfons Grabher, MIT license.
